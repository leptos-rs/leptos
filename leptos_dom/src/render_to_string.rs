...
cfg_if! {
    if #[cfg(not(any(feature = "csr", feature = "hydrate")))] {
        ...
        /// 3) HTML fragments to replace each `<Suspense/>` fallback with its actual data as the resources
        ///    read under that `<Suspense/>` resolve.
        pub fn render_to_stream_undisposed(view: impl FnOnce(Scope) -> Element + 'static) -> (Scope, ScopeDisposer, impl Stream<Item = String>) {
            // create the runtime
            let runtime = create_runtime();

            let ((shell, pending_resources, pending_fragments, serializers, cx), _, disposer) =
                run_scope_undisposed(runtime, {
                    move |cx| {
                        // the actual app body/template code
                        // this does NOT contain any of the data being loaded asynchronously in resources
            ...
            let resources_and_fragments = futures::stream::select(
             ...
             // HTML for the view function and script to store resources
             let bundle = futures::stream::once(async move {
                format!(
                    r#"
                        {shell}
                        <script>
