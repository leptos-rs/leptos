...
#[derive(Clone, Debug)]
pub enum TimeoutHandle {
    /// A standard timeout that fits within the i32 limit of web-sys `set_timeout`
    Single {
        /// The handle returned by the first `setTimeout` call
        first_handle: i32,
    },
    /// A chunked timeout that exceeds the i32 limit of web-sys `set_timeout``
    Chunked {
        /// The handle returned by the first `setTimeout` call
        first_handle: i32,
        /// Whether the timeout has been cancelled. The first timeout will be cancelled,
        /// but subsequent timeouts will check this flag before executing the callback.
...
impl TimeoutHandle {
    ...
    pub fn clear(&self) {
        match self {
            TimeoutHandle::Single { first_handle } => {
                window().clear_timeout_with_handle(*first_handle);
            }
            TimeoutHandle::Chunked {
                first_handle,
                cancelled,
            } => {
...
impl PartialEq for TimeoutHandle {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (
                TimeoutHandle::Single { first_handle: a },
                TimeoutHandle::Single { first_handle: b },
            ) => a == b,
            (
                TimeoutHandle::Chunked {
                    first_handle: a, ..
...
impl std::hash::Hash for TimeoutHandle {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            TimeoutHandle::Single { first_handle } => {
                first_handle.hash(state);
            }
            TimeoutHandle::Chunked { first_handle, .. } => {
                first_handle.hash(state);
            }
        }
    }
...
pub fn set_timeout_with_handle(
    ...
    #[inline(never)]
    fn st(cb: JsValue, millis: i32) -> Result<i32, JsValue> {
    ...
    const MAX_MS_FOR_SINGLE_TIMEOUT: u128 = i32::MAX as u128;
    ...
    fn st_chunked(
        ...
        if remaining_ms <= MAX_MS_FOR_SINGLE_TIMEOUT {
            // Last block, execute the real callback
            let final_cb = move || {
                if !cancelled.load(std::sync::atomic::Ordering::SeqCst) {
                    cb();
                }
            ...
            _ = window().set_timeout_with_callback_and_timeout_and_arguments_0(
                closure_once(final_cb).as_ref().unchecked_ref(),
                remaining_ms as i32,
            );
        } else {
            let next_cb = move || {
                ...
                st_chunked(
                    cb,
                    remaining_ms - MAX_MS_FOR_SINGLE_TIMEOUT,
                    cancelled.clone(),
                );
            };
            _ = window().set_timeout_with_callback_and_timeout_and_arguments_0(
                closure_once(next_cb).as_ref().unchecked_ref(),
                MAX_MS_FOR_SINGLE_TIMEOUT as i32,
            );
        }
    }

    let ms_total = duration.as_millis();
    if ms_total <= MAX_MS_FOR_SINGLE_TIMEOUT {
        let first_handle = st(closure_once(cb), ms_total as i32)?;
        Ok(TimeoutHandle::Single { first_handle })
    } else {
        let cancelled =
        ...
        let first_cb = move || {
            ...
            st_chunked(
                cb,
                ms_total - MAX_MS_FOR_SINGLE_TIMEOUT,
                cancelled_clone,
            );
        };
        let first_handle =
            st(closure_once(first_cb), MAX_MS_FOR_SINGLE_TIMEOUT as i32)?;
        Ok(TimeoutHandle::Chunked {
            first_handle,
            cancelled,
        })
    }
