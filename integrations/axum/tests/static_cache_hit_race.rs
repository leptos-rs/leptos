...
mod tests {
    ...
    async fn cache_hit_during_regeneration_pairs_body_with_headers() {
        let site_root = std::env::temp_dir()
            .join(format!("leptos_axum_cache_hit_race_{}", std::process::id()));
        ...
        'outer: for _ in 0..ROUNDS {
            let reqs = (0..BATCH).map(|_| {
                ...
                tokio::spawn(async move {
                    ...
                    (header_epoch, body_epoch(&html))
                })
            });
            for result in futures::future::join_all(reqs).await {
                let (header_epoch, body_epoch) = result.unwrap();
