...
use pavex::{http::header::{self, HeaderMap, HeaderName, HeaderValue}, request::{RequestHead, body::RawIncomingBody}};
use pavex::http::StatusCode;
use pavex::request::Request;
use pavex::response::body::raw::BoxBody;
...
/// We're creating this type because Pavex doesn't provide one, and this seems
/// easier than trying to convert into the other ones
#[derive(Debug, Clone)]
...
impl PavexRequest{
    // Tell Pavex how to create this type
    pub fn extract(head: &RequestHead, body: RawIncomingBody) -> PavexRequest{
        PavexRequest { head, body }
    }
}
...
async fn render_app_async_helper(
    ...
    let mut res = Response::ok().set_typed_body(html);
...
//                content_type,
//            } => {
//                let mut res = Response::new(match status {
