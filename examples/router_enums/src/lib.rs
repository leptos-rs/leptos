...
use leptos_router::{
    ...
    params::Params,
    MatchNestedRoutes, ParamSegment, Routable, StaticSegment,
};
use tracing::info;
        ...
        <Router>
            ...
            <main>
                <Routes fallback=|| "This page could not be found.">
                    <Route path=StaticSegment(Paths::About) view=About/>
                    <ProtectedRoute
                        path=StaticSegment(Paths::Settings)
                        condition=move || Some(logged_in.get())
                        redirect_path=|| "/"
                        view=Settings
                    />
                    <Route
                        path=StaticSegment(Paths::Home)
...
pub fn ContactRoutes() -> impl MatchNestedRoutes<Dom> + Clone {
    view! {
        <ParentRoute path=StaticSegment(Paths::Contacts) view=ContactList>
            <Route path=StaticSegment(Paths::Contacts) view=|| "Select a contact."/>
